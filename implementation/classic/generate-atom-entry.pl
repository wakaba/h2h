...
my $h2h_p = $gls->create_gls_parser ({
              $fe.'H2H' => '1.0',
            });
my $h2h_doc = $h2h_p->parse_string ($h2h_data);
...
for my $section (@{$h2h_doc->get_elements_by_tag_name_ns ($html, 'body')
  ...
  my $atom_doc = $aimpl->create_atom_entry_document
  ...
  print $entry_file Encode::encode ('utf-8', $ls->write_to_string ($atom_doc));
  close $entry_file;
